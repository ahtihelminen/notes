...
test('a specific note is within the returned notes', async () => {
    ...
    expect(contents).toBe('Browser can execute only Javascript')
})
...
test('a valid note can be added', async () => {
    ...
    const respones = await helper.notesInDb()
    const contents = respones.body.map(r=>r.content)
...
test('a specific note can be viewed', async () => {
    ...
    expect(resultNote).toBe(processedNoteToView)
})
